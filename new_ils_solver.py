...
node_type_selection_probability = {'subtree': 0, 'internal': 10, 'leaf': 10, 'leafs': 10, 'root': 10, 'top': 10, 'bottom': 10, 'level': 10, 'path': 10, 'partial_path': 10, 'partial_path_bottom': 10}
...
class IteratedLocalSearch:
    ...
    def ils_algorithm(self):
        ...
        while True:  # Change the loop condition
            ...
            iteration_counter += 1
            if time.time() - start_time > 10:  # If more than 30 mins have passed, save and break
            ...
            if killer.exit_now and time.time() - start_time > 10:
                output_format_solution = self.convert_to_pace_format(best)
                self.save_solution(
                    instance_name, output_format_solution, best.fitness)
                break
